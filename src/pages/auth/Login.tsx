  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (error?.response?.status === 401) {
        ...
        toast({title:"Invalid email or password. Please try again.",variant:'warning'});
      } else if (error?.response?.status === 404) {
        setError("No account found with this email address.");
        toast({title:"No account found with this email address.",variant:'destructive'});
      } else if (error?.response?.status === 403) {
        setError("Your account has been suspended. Please contact support.");
        toast({title:"Your account has been suspended. Please contact support.",variant:'warning'});
      } else if (error?.response?.status === 429) {
        setError("Too many login attempts. Please try again later.");
        toast({title:"Too many login attempts. Please try again later.",variant:'warning'});
